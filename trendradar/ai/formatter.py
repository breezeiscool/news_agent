...
def _render_ai_analysis_markdown_like(
    ...
    if result.core_trends:
        lines.extend(["**今日核心研判**", _format_list_content(result.core_trends), ""])

    if result.sentiment_controversy:
        lines.extend(
            ["**客户与公司动态**", _format_list_content(result.sentiment_controversy), ""]
        )
    ...
    if result.rss_insights:
        lines.extend(
            ["**专业源深度洞察**", _format_list_content(result.rss_insights), ""]
        )

    if result.outlook_strategy:
        lines.extend(
            ["**值得跟踪的线索**", _format_list_content(result.outlook_strategy), ""]
        )
...
def render_ai_analysis_dingtalk(result: AIAnalysisResult) -> str:
    ...
    if result.core_trends:
        lines.extend(
            ["#### 今日核心研判", _format_list_content(result.core_trends), ""]
        )
...
def render_ai_analysis_plain(result: AIAnalysisResult) -> str:
    ...
    if result.rss_insights:
        lines.extend(["[专业源深度洞察]", _format_list_content(result.rss_insights), ""])

    if result.outlook_strategy:
        lines.extend(["[值得跟踪的线索]", _format_list_content(result.outlook_strategy), ""])
...
def render_ai_analysis_telegram(result: AIAnalysisResult) -> str:
    ...
    if result.core_trends:
        lines.extend(["<b>今日核心研判</b>", _escape_html(_format_list_content(result.core_trends)), ""])

    if result.sentiment_controversy:
        lines.extend(["<b>客户与公司动态</b>", _escape_html(_format_list_content(result.sentiment_controversy)), ""])

    if result.signals:
        lines.extend(["<b>异动与弱信号</b>", _escape_html(_format_list_content(result.signals)), ""])

    if result.rss_insights:
        lines.extend(["<b>专业源深度洞察</b>", _escape_html(_format_list_content(result.rss_insights)), ""])

    if result.outlook_strategy:
        lines.extend(["<b>值得跟踪的线索</b>", _escape_html(_format_list_content(result.outlook_strategy)), ""])
...
def render_ai_analysis_html_rich(result: AIAnalysisResult) -> str:
    ...
    def _block(title, text, extra_class=""):
        content_html = _escape_html(_format_list_content(text)).replace("\n", "<br>")
        return f"""
                    <div class="ai-block{extra_class}">
                        <div class="ai-block-title">{title}</div>
                        <div class="ai-block-content">{content_html}</div>
                    </div>"""

    # 主栏：核心研判（阅读主体）+ 客户与公司动态；侧栏：值得跟踪的线索（不打断主线阅读）
    main_html = ""
    if result.core_trends:
        main_html += _block("今日核心研判", result.core_trends, " ai-block-core")
    if result.sentiment_controversy:
        main_html += _block("客户与公司动态", result.sentiment_controversy)
    if result.signals:
        main_html += _block("异动与弱信号", result.signals)
    if result.rss_insights:
        main_html += _block("专业源深度洞察", result.rss_insights)

    side_html = ""
    if result.outlook_strategy:
        side_html += _block("📌 值得跟踪的线索", result.outlook_strategy)
    if result.standalone_summaries:
        summaries_text = _format_standalone_summaries(result.standalone_summaries)
        if summaries_text:
            side_html += _block("独立源点速览", summaries_text)

    ai_html = """
                <div class="ai-section">
                    <div class="ai-section-header">
                        <div class="ai-section-title">✨ AI 情报分析</div>
    ...
    if side_html:
        ai_html += f"""
                    <div class="ai-layout">
                        <div class="ai-main">{main_html}
                        </div>
                        <aside class="ai-side">{side_html}
                        </aside>
                    </div>"""
    else:
        ai_html += f"""
